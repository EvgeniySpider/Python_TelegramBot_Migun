...
class StatsRepository:
    ...
    async def increment_metric(self, metric_name: MetricColumn) -> None:
        ...
        # Формируем SQL c безопасной подстановкой колонки
        query = f"""
            ...
            SET {metric_name} = events_botstatistics.{metric_name} + 1
        """

        async with self.db.connection() as conn:
            await conn.execute(query, today)
